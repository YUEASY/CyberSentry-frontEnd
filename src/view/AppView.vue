        ...
        <div class="app-list-section">
          ...
          <a-spin :loading="loading" class="app-list-spinner">
            <a-list :scrollbar="true" :bordered="false" class="app-list">
              <a-list-item v-for="d in paginatedDatas" :key="d.app_id" :bordered="false" class="app-list-item">
                <div class="list-item" @click="() => selectApp(d.app_id)"
                  :class="{ 'active': detaileData.app_id === d.app_id }">
                  <div class="icon-column">
                    <img :width="50" :src="d.icon_path === '' ? defineIcon : `/api/api/get_image/${d.icon_path}`"
                      class="app-icon" :alt="d.app_name" />
                  </div>
                  <div class="name-column app-name">{{ d.app_name }}</div>
                  <div class="app-element">
                    <span :style="{ color: getResourceColor(d.cpu_usage) }">{{ d.cpu_usage?.toFixed(2) }}%</span>
                  </div>
                  <div class="app-element">
                    <span :style="{ color: getResourceColor((d.memory_usage_mb / 1024 / 16)) }">
                      {{ (d.memory_usage_mb / 1024 / 16)?.toFixed(2) }}%
                    </span>
                  </div>
                  <div class="app-element">{{ d.disk_io_read?.toFixed(2) }}</div>
                  <div class="app-element">{{ d.disk_io_write?.toFixed(2) }}</div>
                  <div class="app-element">
                    <div class="power-badge" :style="{ backgroundColor: getPowerLevelColor(d.power_use_level) }">
                      {{ d.power_use_level }}
                    </div>
                  </div>
                </div>
          ...
          <div v-if="Object.keys(detaileData).length > 0" class="app-detail-content">
            ...
            <div class="app-detail-header">
              ...
              <div class="app-info">
                ...
                <div class="app-power">
                  <span>电源消耗评级:</span>
                  <div class="power-badge" :style="{ backgroundColor: getPowerLevelColor(detaileData.power_use_level) }">
            ...
            <div class="resource-metrics">
              <div class="resource-card">
                ...
                <div class="resource-value" :style="{ color: getResourceColor(detaileData.cpu_usage) }">
                  {{ detaileData.cpu_usage?.toFixed(2) }}%
                </div>
              ...
              <div class="resource-card">
                ...
                <div class="resource-value" :style="{ color: getResourceColor((detaileData.memory_usage_mb / 1024 / 16)) }">
                  {{ (detaileData.memory_usage_mb / 1024 / 16)?.toFixed(2) }}%
                </div>
                <div class="resource-bar">
                  <div class="resource-progress" :style="{ 
